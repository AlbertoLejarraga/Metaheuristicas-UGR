...
def repartirClasesNormalizadas(etiqueta, particiones=5):
    ...
    for i in range(particiones):
        listaResul.append([])
    #normalizar los datos antes de introducirlos en las sublistas
...
def normalizar(lista):
    ...
    minimos=[sys.maxint for i in range(lista[1].__len__()-1)]
    maximos=[-sys.maxint-1 for i in range(lista[1].__len__()-1)]
    listaResul=[]

    for ejemplos in lista:
        i=0
        for valor in ejemplos[:-1]:
            if valor>maximos[i]: maximos[i]=valor
            if valor<minimos[i]: minimos[i]=valor
            i+=1
    #a partir de los maximos valores normalizo con la formula [(valor-minimos[i])/(maximos[i]-minimos[i])]
    for ejemplos in lista:
        listado=[]
        i=0
        for valor in ejemplos[:-1]:
            listado.append((valor-minimos[i])/(maximos[i]-minimos[i]))
            i+=1
        listado.append(ejemplos[-1])
...
def distanciaEuclidea(elemento1,elemento2):
    suma=0
    dist = numpy.linalg.norm(elemento1-elemento2)
...
def evaluarSolucion(pesos, particion, conjuntoEntrenamiento, alfa=0.5):
    ...
    for valor in pesos:
        if valor<0.2:suma+=1
    tasa_red=100.0*suma/pesos.__len__()-1
...
def generarVecino(solucion,distNormal,caractAModificar):
    solucionAux=[valor for valor in solucion]
    nuevoValor=distNormal.pop()
    solucionAux[caractAModificar]+=nuevoValor
    if solucionAux[caractAModificar]>1:solucionAux[caractAModificar]=1
    elif solucionAux[caractAModificar]<0:solucionAux[caractAModificar]=0
    return solucionAux
